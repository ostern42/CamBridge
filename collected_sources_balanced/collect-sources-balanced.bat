...
echo Version: GUI Development v0.4.2+
...
(
echo # CamBridge Context v0.4.2
echo © 2025 Claude's Improbably Reliable Software Solutions
echo.
echo ## Aktueller Stand
echo - v0.4.0: GUI Framework mit Dashboard ^(2x committed!^)
echo - v0.4.1: Settings-Page komplett implementiert
echo - Nächste: Dead Letters Implementation ^(v0.4.2^)
echo.
echo ## PROJECT_WISDOM.md
echo Enthält das MAGIC WORDS SYSTEM:
echo - "CAMBRIDGE INIT" für automatischen Start
echo - "WISDOM: [text]" für Live-Updates
echo - "CLAUDE: [text]" für persönliche Notizen
echo.
...
echo ## Tech Stack
echo - .NET 8, C# 12
echo - WPF mit ModernWpfUI 0.9.9 ^(NICHT WinUI3!^)
echo - CommunityToolkit.Mvvm 8.2.2
echo - ASP.NET Core Minimal API
echo - fo-dicom für DICOM
echo.
echo ## Nächste Aufgabe: Dead Letters ^(v0.4.2^)
echo - Vollständige Dead Letters Page mit Liste
echo - HttpApiService für /api/deadletters
echo - Retry/Delete/Export Funktionen
echo - Batch-Operationen
echo - Integration mit DeadLetterQueueService
echo.
echo ## Wichtige Patterns
echo - MVVM mit ObservableObject
echo - Async/Await für I/O
echo - Dependency Injection überall
echo - Clean Architecture
echo - KISS-Prinzip beachten!
...
echo.
echo Fokus: Dead Letters Implementation
echo Version: v0.4.2 (Next Phase)
echo PROJECT_WISDOM.md: INKLUDIERT!
echo.
echo Inhalt:
