...
// Modified: 2025-07-03
// Status: System Fields UI + DICOM Tag Browser Fix Complete
...
namespace CamBridge.Config.ViewModels
{
    ...
    public partial class MappingEditorViewModel : ViewModelBase
    {
        ...
        public ObservableCollection<SourceFieldInfo> SystemFields { get; } = new();
        public ObservableCollection<SourceFieldInfo> FileInfoFields { get; } = new();
        ...
        private void InitializeSourceFields()
        {
            ...
            // System fields (NEW! - generated by CamBridge)
            SystemFields.Add(new SourceFieldInfo("ConversionDateTime", "Conversion Date/Time", "datetime"));
            SystemFields.Add(new SourceFieldInfo("ConversionDate", "Conversion Date Only", "date"));
            SystemFields.Add(new SourceFieldInfo("ConversionTime", "Conversion Time Only", "time"));
            SystemFields.Add(new SourceFieldInfo("MachineName", "Computer Name", "string"));
            SystemFields.Add(new SourceFieldInfo("UserName", "Windows User", "string"));
            SystemFields.Add(new SourceFieldInfo("PipelineName", "Pipeline Name", "string"));
            SystemFields.Add(new SourceFieldInfo("CamBridgeVersion", "CamBridge Version", "string"));
            SystemFields.Add(new SourceFieldInfo("InstitutionName", "Institution", "string"));
            SystemFields.Add(new SourceFieldInfo("CaptureAge", "Days Since Capture", "number"));
            SystemFields.Add(new SourceFieldInfo("CaptureAgeHours", "Hours Since Capture", "number"));

            // File information fields (NEW!)
            FileInfoFields.Add(new SourceFieldInfo("SourceFileName", "Original File Name", "string"));
            FileInfoFields.Add(new SourceFieldInfo("SourceFileSize", "File Size (bytes)", "number"));
            FileInfoFields.Add(new SourceFieldInfo("SourceFileExtension", "File Extension", "string"));
        }
                ...
                foreach (var rule in SelectedMappingSet.Rules)
                {
                    try
                    {
                        if (testData.TryGetValue(rule.SourceField, out var sourceValue))
                        {
                            ...
                            results.AppendLine($"[OK] {rule.SourceField} -> {rule.DicomTag}: '{transformedValue}'");
                            successCount++;
                        }
                        else
                        {
                    ...
                    {
                        results.AppendLine($"[X] {rule.SourceField}: Error - {ex.Message}");
                        errorCount++;
                    }
                }
        ...
        private void BrowseDicomTags()
        {
            ...
            if (dialog.ShowDialog() == true && dialog.SelectedTag != null)
            {
                ...
                // Update the display name from the selected tag
                // NOTE: This requires adding SelectedTagName property to DicomTagBrowserDialog
                // if (dialog.SelectedTagName != null)
                // {
                //     SelectedRule.DisplayName = dialog.SelectedTagName;
                // }
        ...
        public void AddRuleFromField(SourceFieldInfo fieldInfo)
        {
            ...
            string dicomTag = fieldInfo.FieldName switch
            {
                ...
                // NEW System Field mappings:
                "ConversionDateTime" => "(0008,002A)",  // Acquisition DateTime
                "ConversionDate" => "(0008,0023)",      // Content Date  
                "ConversionTime" => "(0008,0033)",      // Content Time
                "MachineName" => "(0008,1010)",         // Station Name
                "UserName" => "(0008,1070)",            // Operators' Name
                "PipelineName" => "(0008,103E)",        // Series Description
                "InstitutionName" => "(0008,0080)",     // Institution Name
                "CaptureAge" => "(0018,1151)",          // Temporal Position Identifier
                "SourceFileName" => "(0008,2111)",      // Derivation Description
                "SourceFileSize" => "(0028,0120)",      // Pixel Padding Value (misused but available)
            ...
            var newRule = new MappingRule
            {
                ...
                SourceType = QRBridgeFields.Contains(fieldInfo) ? "QRBridge"
                           : ExifFields.Contains(fieldInfo) ? "EXIF"
                           : SystemFields.Contains(fieldInfo) ? "System"
                           : FileInfoFields.Contains(fieldInfo) ? "FileInfo"
                           : "Unknown",
